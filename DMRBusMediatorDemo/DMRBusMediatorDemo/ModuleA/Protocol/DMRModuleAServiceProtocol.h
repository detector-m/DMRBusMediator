...
#import "DMRModuleAItemProtocol.h"
...
/**
 * @protocol ModuleAXXXServiceProtocol (服务协议)
 *  ModuleA对外开放的xxxService，向外提供XXX服务，所有接口需要require实现
 */
@protocol DMRModuleAServiceProtocol <NSObject>

//服务接口1: 传入Message显示一个alert，并提供cancel和confirm的回调，这个例子是同步回调
//如果是异步回调，有实现这个接口的Impl自己去控制，保证多线程安全
- (void)moduleAShowAlertWithMessage:(nonnull NSString *)message cancelAction:(void (^ _Nullable)(NSDictionary * _Nullable info))cancelAction confirmAction:(void (^ _Nullable)(NSDictionary * _Nullable info))confirmAction;
